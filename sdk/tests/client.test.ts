...
describe("AgentPlane", () => {
  it("throws if no API key provided", () => {
    ...
    expect(() => new AgentPlane()).toThrow("API key is required");
  });
  ...
  it("accepts API key in options", () => {
    const client = new AgentPlane({ apiKey: "ap_live_test1234567890abcdef12345678" });
    expect(client).toBeDefined();
  });
  ...
  it("hides credentials in toJSON", () => {
    ...
    expect(json.baseUrl).toBe("https://agentplane.vercel.app");
  });
  ...
  describe("_request", () => {
    it("sends authorization header and parses JSON response", async () => {
      ...
      expect(url).toBe("https://agentplane.vercel.app/api/agents");
